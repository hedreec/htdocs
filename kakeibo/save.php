...
if ($cnt === 0) {
  header('Location: index.php?ng=' . urlencode('行がありません。'));
  exit;
}
...
try {
  ...
  for ($i=0;$i<$cnt;$i++) {
    ...
    $stmt->execute([$d, $c, $p, (int)$a]);
    $n++;
  }
  $pdo->commit();
  if ($n === 0) {
    header('Location: index.php?ng=' . urlencode('有効な行がありませんでした。'));
  } else {
    header('Location: index.php?ok=1');
  }
} catch (Throwable $e) {
  $pdo->rollBack();
  header('Location: index.php?ng=' . urlencode('保存に失敗しました。' . $e->getMessage()));
}
